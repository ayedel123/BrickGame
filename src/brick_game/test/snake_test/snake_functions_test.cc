...
TEST_F(SnakeFunctionsTest, case_8) {
  ...
  s21::Snake snake = s21::Snake(&game_info, field);
  snake.SpawnApple();
  ASSERT_EQ(field[snake.game_info->next_brick.y][snake.game_info->next_brick.x],
...
TEST_F(SnakeFunctionsTest, case_9) {
  ...
  for (int i = 0; i < 196; i++) {
    snake.SpawnApple();
    ASSERT_EQ(
        field[snake.game_info->next_brick.y][snake.game_info->next_brick.x],
        snake.game_info->next_brick.color);
  }
  DeleteField(game_info.field, GAME_WINDOW_HEIGHT);
}
...
TEST_F(SnakeFunctionsTest, case_11) {
  ...
  GameInfo game_info;
  s21::Snake snake = s21::Snake(&game_info, field);
  GameState state = snake.SnakeHandleCollision(
      snake.game_info->current_brick.color, snake, kMoving);
...
TEST_F(SnakeFunctionsTest, case_12) {
  ...
  s21::Snake snake = s21::Snake(&game_info, field);
  GameState state =
      snake.SnakeHandleCollision(COLLIDE_WITH_BORDER, snake, kMoving);
  ASSERT_EQ(state, kGameOver);
  DeleteField(game_info.field, GAME_WINDOW_HEIGHT);
}
