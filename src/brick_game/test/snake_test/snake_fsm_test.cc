...
TEST_F(SnakeFSMTest, case_1) {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    ASSERT_EQ(snake.game_info
                  ->field[GAME_WINDOW_HEIGHT / 2 - i][GAME_WINDOW_WIDTH / 2],
              snake.body.front()->color);
  }
...
TEST_F(SnakeFSMTest, case_5) {
  ...
  ASSERT_EQ(snake.game_info->field[snake.game_info->next_brick.y]
                                  [snake.game_info->next_brick.x],
            snake.game_info->next_brick.color);
...
TEST_F(SnakeFSMTest, case_13) {
  ...
  s21::Snake snake = s21::Snake(&game_info, field);
  GameState state = kMoving;
  field[GAME_WINDOW_HEIGHT / 2 + 1][GAME_WINDOW_WIDTH / 2] =
      snake.game_info->next_brick.color;
  SnakeUpdateCurrentState(snake, &state, kMoveDown);
  ASSERT_EQ(state, kSpawn);
  for (int i = 0; i < 5; i++) {
    ASSERT_EQ(snake.body[i]->x, GAME_WINDOW_WIDTH / 2);
    ASSERT_EQ(snake.body[i]->y, GAME_WINDOW_HEIGHT / 2 - i + 1);
    ASSERT_EQ(snake.game_info->field[GAME_WINDOW_HEIGHT / 2 - i + 1]
                                    [GAME_WINDOW_WIDTH / 2],
              snake.body.front()->color);
  }
