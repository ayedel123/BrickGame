#ifndef CPP3_BRICKGAME_V2_0_1_SRC_BRICK_GAME_SNAKE_SNAKE_CLASS_H_
#define CPP3_BRICKGAME_V2_0_1_SRC_BRICK_GAME_SNAKE_SNAKE_CLASS_H_
...
class Snake {
 public:
  ...
  Snake(GameInfo *game_info, int **field) : Snake() {
    ...
    InitGameInfo(this->game_info, field, game_speed, game_acceleration,
                 BRICK_TYPES_COUNT, "snake_record");
  ...
  GameState SnakeHandleCollision(int col, Snake &snake, GameState state) {
    if (col == game_info->next_brick.color) {
      ...
      state = kSpawn;
    } else if (col == COLLIDE_WITH_BORDER ||
               col == game_info->current_brick.color) {
      state = kGameOver;
    }
  ...
  int MoveBody(int direction, bool ignore_collision) {
    if (body.empty()) return COL_STATE_NO;
  ...
  int AddPoints(int apple) {
    ...
    if (game_info->points < 200) game_info->points += points;
